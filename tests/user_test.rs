...
async fn test_admin_message_flow() {
    ...
    let res = build_waku_message(alice_ga_msg.clone());
    ...
    let res = build_waku_message(bob_kp_message.clone());
    assert!(res.is_ok(), "Failed to build waku message");
    ...
    let res = build_waku_message(alice_welcome_message.clone());
    assert!(res.is_ok(), "Failed to build waku message");
...
async fn join_group(alice: &mut User, bob: &mut User, group_name: String) -> UserAction {
    // Alice send Group Announcement msg to Bob
    let res = alice.prepare_admin_msg(group_name.clone()).await;
    assert!(res.is_ok(), "Failed to prepare admin message");
    let alice_ga_msg = res.unwrap();
...
async fn test_remove_user_flow() {
    ...
    let _ = join_group(&mut alice, &mut bob, group_name.clone()).await;
    let bob_group = bob.get_group(group_name.clone()).unwrap();
    assert!(
        bob_group.is_mls_group_initialized(),
        "MLS group is not initialized"
    ...
    let commit_action = join_group(&mut alice, &mut carol, group_name.clone()).await;
    ...
    let commit_message = build_waku_message(pmt);
