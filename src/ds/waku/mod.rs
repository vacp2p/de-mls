...
pub fn build_content_topics(conversation_name: &str) -> Vec<String> {
    SUBTOPICS
        .iter()
        .map(|subtopic| build_content_topic(conversation_name, GROUP_VERSION, subtopic))
        .collect()
}

/// Build the content topic string: `/{conversation_name}/{version}/{subtopic}/proto`
pub fn build_content_topic(conversation_name: &str, group_version: &str, subtopic: &str) -> String {
    format!("/{conversation_name}/{group_version}/{subtopic}/proto")
}
...
impl WakuDeliveryService {
    ...
    fn do_publish(
        ...
        let content_topic = build_content_topic(&pkt.conversation_id, GROUP_VERSION, &pkt.subtopic);
    ...
    fn parse_event(data: &str) -> Option<InboundPacket> {
        ...
        debug!("Inbound message: group={conversation_id} subtopic={subtopic}");
