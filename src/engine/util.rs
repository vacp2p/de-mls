...
//! the deterministic self-leave proposal id, a borrowed member-set view, and
//! the member a proposal targets.
...
pub(crate) fn self_leave_proposal_id(member_id: &[u8]) -> u32 {
...
pub(crate) fn target_member_id_of(request: &ConversationUpdateRequest) -> Option<Vec<u8>> {
    match request.payload.as_ref()? {
        conversation_update_request::Payload::MemberInvite(m) => {
            (!m.member_id.is_empty()).then(|| m.member_id.clone())
        }
